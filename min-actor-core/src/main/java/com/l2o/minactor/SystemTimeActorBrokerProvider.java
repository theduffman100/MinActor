...
/**
 * A pooled, system time implementation of ActorBrokerProvider. ActorBrokers may
 * share a common thread depending on how they obtain the broker. They are thus
 * expected to behave properly and not block while handling an event.
 */
public class SystemTimeActorBrokerProvider extends BaseActor<Runnable> implements ActorBrokerProvider {
  ...
  /**
   * Obtain a pooled broker. Brokers will handle up to MaxHandlersPerThread
   * handlers. When all existing brokers reach that limit, new brokers will be
   * allocated. The actor will first be attributed a temporary threadless broker,
   * then the actual pooled broker will be attributed in an asynchronous event and
