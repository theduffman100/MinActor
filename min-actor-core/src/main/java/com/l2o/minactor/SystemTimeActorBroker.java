...
/**
 * System time implementation of the ActorBroker interface.
 * This implementation will handle timers efficiently by parking the thread while waiting
 *   for either the next time event or an asynchronous event.
 */
public class SystemTimeActorBroker extends BaseActorBroker implements ActorBroker, Runnable {
	...
	while (true) {
		...
		if (nextTimeEvent != null) {
		    	...
		    	if (remaining <= 0) {
		    	    ...
		    	    // Otherwise park the thread until there is an asynchronous event or the time event is expired
	...
	TimeEventWrapper<EVENTTYPE> wrapper = new TimeEventWrapper<EVENTTYPE>(event, handler, System.currentTimeMillis() + time);
	if (Thread.currentThread() == thread) {
	    // Covers most of the cases
	    timeEvents.add(wrapper);
	}
	else {
	    // We're not in the actor's thread and the time event priority queue is not thread-safe.
	    //   Post it as an asynchronous event and the run() method will safely add it to the priority queue.
