...
/**
 * This base class can be derived to provide instances of an asynchronous interface with calls being
 *   declared as synchronous (either returning a result or suspending it and send the result later).
 *   
 *   The asynchronous interface methods should return either CallResult or void.
 *   
 *   Methods in the interface and sub-class must have the same name and parameter types and be visible.
 *   
 *   If an interface method returns void (making it impossible to hook exception and timeout
 *   handlers) then the method will be called in a pure async fashion. Otherwise the implementation
 *   can return void while the interface method returns a CallResult, in which case the caller
 *   can hook exception and timeout handlers.
 *   
 *   The implementation can return the value after its execution by calling suspendCall()
 *   and using the returned object later. In this case its return value will be ignored (the implementation
 *   method can return void).
 *
 * @param <INTERFACE> The proxy interface. All methods in that proxy should return CallResults if the result is
 *      returned right away, or void if the result is to be sent later.
 */
public abstract class DelegateActor<INTERFACE> extends BaseActor<CallWithParam<Invocation, Object>> implements InvocationHandler {
    ...
    /**
     * Gets the current call object to be able to send the result later.
     * This prevents the result from being sent immediately after the invocation.
